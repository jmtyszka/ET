function varargout = ET_prepare(varargin)
% ET_PREPARE MATLAB code for ET_prepare.fig
%      ET_PREPARE, by itself, creates a new ET_PREPARE or raises the existing
%      singleton*.
%
%      H = ET_PREPARE returns the handle to a new ET_PREPARE or the handle to
%      the existing singleton*.
%
%      ET_PREPARE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ET_PREPARE.M with the given input arguments.
%
%      ET_PREPARE('Property','Value',...) creates a new ET_PREPARE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ET_prepare_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ET_prepare_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ET_prepare

% Last Modified by GUIDE v2.5 07-Oct-2013 00:17:55

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ET_prepare_OpeningFcn, ...
                   'gui_OutputFcn',  @ET_prepare_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ET_prepare is made visible.
function ET_prepare_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ET_prepare (see VARARGIN)

% Choose default command line output for ET_prepare
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes ET_prepare wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = ET_prepare_OutputFcn(hObject, eventdata, handles) 
...
v_out = VideoWriter(strrep(fullfile(handles.dir_name,handles.fname),'.mp4','_CAL.mp4'),'MPEG-4');
...
% write a new video object
v_out = VideoWriter(strrep(fullfile(handles.dir_name,handles.fname),'.mp4','_VAL.mp4'),'MPEG-4');
set(v_out,'FrameRate',handles.FrameRate,'Quality',100);
open(v_out);
for k = 1 : numFrames
...
% write a new video object
v_out = VideoWriter(strrep(fullfile(handles.dir_name,handles.fname),'.mp4','_GAZE.mp4'),'MPEG-4');
set(v_out,'FrameRate',handles.FrameRate,'Quality',100);
open(v_out);
for k = 1 : numFrames
    if GAZEstart+k-1>handles.nFrames
        break
    end
